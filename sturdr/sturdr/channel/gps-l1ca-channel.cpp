...
GpsL1caChannel::GpsL1caChannel(
  ...
  channel_msg_.ChannelNum = channel_num;
  channel_msg_.Constellation = GnssSystem::GPS;
  channel_msg_.Signal = GnssSignal::GPS_L1CA;
  channel_msg_.ChannelStatus = ChannelState::IDLE;
  nav_msg_.ChannelNum = channel_num;
  nav_msg_.Constellation = GnssSystem::GPS;
  nav_msg_.Signal = GnssSignal::GPS_L1CA;
...
// // *=== start ===*
// void GpsL1caChannel::start() {
//   log_->debug("GpsL1caChannel::start starting thread!");
//   // thread_ = std::thread(&GpsL1caChannel::run, this);
//   thread_ = std::make_shared<std::thread>(&GpsL1caChannel::run, this);
// }

// // *=== join ===*
// void GpsL1caChannel::join() {
//   if (thread_->joinable()) {
//     thread_->join();
//   }
// }
...
void GpsL1caChannel::Acquire() {
  ...
  if (metric > conf_.acquisition.threshold) {
    ...
    channel_msg_.ChannelStatus = ChannelState::IDLE;
    log_->info(
        "Acquisition for GPS{} on {} failed, resetting to idle!", channel_msg_.SVID, channel_id_);
  }
}
...
void GpsL1caChannel::Track() {  // make sure samp_remaining_ > 0 to start
  ...
  while (samp_remaining_ > 0) {
    if (samp_processed_ < total_samp_) {
      ...
      Eigen::Vector<double, 5> x = filt_.Run(chip_err, phase_err, freq_err);
      rem_carr_phase_ = std::fmod(x(0), navtools::TWO_PI<double>);
      carr_doppler_ = x(1);
      carr_jitter_ = x(2);
      rem_code_phase_ = x(3) - static_cast<double>(PDI_) * GPS_L1CA_CODE_SIZE;
      code_doppler_ = x(4) + kappa_ * (x(1) + t * x(2));
      nco_carr_freq_ = intmd_freq_rad_ + carr_doppler_;
      nco_code_freq_ = GPS_L1CA_CODE_FREQ + code_doppler_;
...
bool GpsL1caChannel::DataBitSync() {
  ...
  if ((P_.real() > 0.0) && (P_old_.real() < 0.0)) {
    ...
    if (cnt_ > conf_.tracking.min_converg_time_ms) {
      ...
      uint8_t ratio = sorted_arr[0] / sorted_arr[1];
...
void GpsL1caChannel::Demodulate() {
  ...
  if (gps_lnav_.NextBit(bit)) {
    ...
    nav_msg_.ToW = channel_msg_.ToW - 0.02;
  ...
  if (gps_lnav_.AreEphemeridesParsed()) {
    ...
    log_->debug("{}", gps_lnav_.GetEphemerides());
  }
}
